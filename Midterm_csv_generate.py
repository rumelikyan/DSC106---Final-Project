...
# Define exam duration in minutes for Midterm 1 (90 minutes)
MIDTERM1_DURATION = 90
...
# List to hold merged Midterm 1 data for all students
midterm1_data_list = []

for student_dir in student_dirs:
    student_id = os.path.basename(student_dir)
    # Folder name is exactly "Midterm 1" (with a space)
    midterm1_dir = os.path.join(student_dir, "Midterm 1")
    if not os.path.isdir(midterm1_dir):
        print(f"Midterm 1 folder not found for {student_id}. Skipping...")
        continue

    sensor_dfs = {}
    for sensor_name, filename in SENSORS.items():
        file_path = os.path.join(midterm1_dir, filename)
        if os.path.exists(file_path):
            try:
                df_sensor = read_sensor_data(file_path, sensor_name)
                df_sensor_agg = aggregate_to_minutes(df_sensor, MIDTERM1_DURATION)
                sensor_dfs[sensor_name] = df_sensor_agg
            except Exception as e:
                print(f"Error processing {sensor_name} for {student_id}: {e}")
        else:
            print(f"{filename} not found for {student_id} in Midterm 1.")
    ...
    if "EDA" in sensor_dfs:
        ...
        df_student["Exam"] = "Midterm 1"
        midterm1_data_list.append(df_student)
    else:
        print(f"No EDA data for {student_id}; skipping merging for this student.")

# Concatenate all students' Midterm 1 data
if midterm1_data_list:
    df_midterm1 = pd.concat(midterm1_data_list, ignore_index=True)
    # Reorder columns: Student, Exam, Minute, then sensor columns (EDA, BVP, TEMP, HR)
    desired_cols = ["Student", "Exam", "Minute", "EDA", "BVP", "TEMP", "HR"]
    existing_cols = [col for col in desired_cols if col in df_midterm1.columns]
    df_midterm1 = df_midterm1[existing_cols]
    # Save to CSV
    df_midterm1.to_csv("merged_midterm1_data.csv", index=False)
    print("Merged Midterm 1 data for all students has been saved to merged_midterm1_data.csv.")
else:
    print("No Midterm 1 data was merged.")
